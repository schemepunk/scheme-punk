...
module.exports = async function transformFactory(
  pluginName,
  molotovOptions = {overrides: {}, cocktailClasses: []}
) {
  // eslint-disable-line max-len
  const extendedClass = await setUpPromise(pluginName, molotovOptions);

  return class schemePunkTransform extends extendedClass {
    // eslint-disable-line max-len
     ...
     */
    transform(value) {
      return super.transform(value).then((xformedValue) => {
