...
module.exports = class schemePunkDestination {
  constructor(options, transformedValue, scheme) {
    ...
    this.scheme = scheme;
    this.destinationTemplate = options.DestinationTemplate;
   ...
   */
  static promoteActiveToNewScheme(scheme) {
    scheme.newScheme = scheme.activeScheme;
  }
  /**
  ...
  validateDestinationTarget() {
    if ({}.hasOwnProperty.call(this.destination, this.target)) {
      return true;
    }
    return false;
  }
  ...
  writeDestinationTarget() {
    ...
    _.set(this.getDestination(), this.getTarget(), this.getValue());
  }

  getWrittenDestination() {
    return this.destination;
  }
};
