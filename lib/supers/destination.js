...
module.exports = class destinationSuperBase {
   ...
   */
  async init(options, transformedValue, scheme, holdOvers, callPath = '', templateObject = false) {
   ...
   *
   */
  async writeDestinationTarget() {
    // Check to see if destination does not yet exist.
    const destinationItem = await this.getDestination();
    const target = await this.getTarget();
    const value = await this.getValue();
    return _.set(this.scheme[destinationItem], target, value);
  }
  ...
  /**
   * Sets the template object for this destination.
   *
  ...
  /**
   * Gets the template object for this destination.
   *
   * @returns {Object}
   *   Returns the template object passed to this destination.
   */
  getTemplateObject() {
