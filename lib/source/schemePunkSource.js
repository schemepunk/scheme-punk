...
function setUpPromise(pluginName, molotovConfig) {
  ...
  if (pluginName) {
    setUp = Promise.resolve(molotov.getMolotov().resolve().source[pluginName]); // Ensures we get any superOverrides from config.
  }
  else {
    setUp = Promise.resolve(molotov.mixSupers().getMolotov().getSupers().source);
  }
