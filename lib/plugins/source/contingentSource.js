...
module.exports = (superclass) => class extends superclass {
  /**
     ...
     *   This string represents a path in the contigency object that when
     *     specified will return a single value from the contigency object
     *       and not the contigency object itself.
     * @param {Object} scheme
     *   The scheme.
  ...
  /**
     ...
     */
  getSwitches(path) {
    return _.get(this.getContingencies(), path, false);
  }
  ...
  async getContingencyProducts(items) {
    ...
    const productSetter = {
      static: (itemsKey) => _.set(contingencySource, itemsKey.path, itemsKey.value),
      dynamic: (itemsKey) => _.set(
        contingencySource,
        itemsKey.path,
        _.get(origin, itemsKey.target)
      ),
      holdOver: (itemsKey) => _.set(tmpHoldOvers, itemsKey.path, itemsKey.holdover),
    };

    Object.keys(items).forEach((key) => {
      // these items will be static, dynamic, or holdovers.
      productSetter[items[key].type](items[key]);
    }, this);
  ...
  async setProductHoldovers(products) {
    ...
    if (!Object.keys(tmpHoldOvers).length) {
      await this.setHoldOvers(_.merge({}, holdOvers, tmpHoldOvers));
      return;
    }
