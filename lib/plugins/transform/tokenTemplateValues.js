...
module.exports = (superclass) => class extends superclass {
  ...
  /**
     ...
     *     need a prependValue in order to work with this transform.
     *
     * @param value
  ...
  */
  async transform(value) {
    ...
    if (Object.keys(holdOvers).length) {
      ...
      tokens = {...tokens, ...holdOvers};
    }
    if (this.options.named) {
      const tmpTokens = {...this.options.tokens};
      // We are using tokens, which are reassignments of values
      // to new names.
      ...
      tokens = {...tokens, ...tmpTokens};
    }
    else {
      // we are not using tokens.
      tokens = {...tokens, ...tmpValue};
    }
    let template;
    let partials = {};
    ...
    if (this.getTemplateObject()) {
      ...
      if (_.get(this.options, 'template.targetPartial', false)) {
        destinationTemplate = _.get(
          objTemplate,
          this.options.template.targetPartial
        );
      }
      else if (_.get(this.options, 'template.filterPartials', false)) {
        // Render only a targeted element of the destination
        // template object.
      ...
      if (!destinationTemplate) {
        throw new Error(
          'Using templated object with no destination template value.'
        );
      }
      template = await destinationTemplate;
    }
    else {
      ...
      template = await this.constructor._getFile(templatePath);
    }

    if (
      _.isEmpty(tokens)
        && !_.get(this.options, 'template.renderEmptyTokens', true)
    ) {
      return super.transform({});
    }

    if (this.options.json) {
      if (mustache.render(template, tokens)) {
        return super.transform(
          JSON.parse(mustache.render(template, tokens, partials))
        );
      }
    }
    ...
    if (this.options.unescape) {
      return super.transform(
        mustache.render(template, tokens, partials).replace(/\\"/g, '"')
      );
    }

    return super.transform(mustache.render(template, tokens, partials));
  }
};
