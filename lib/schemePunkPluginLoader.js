...
if (config.has('schemePunk')) {
  ...
  Object.keys(superConfig).forEach((currentValue) => {
    if (_.get(config.schemePunk[currentValue])) {
      if (_.get(config.schemePunk[currentValue].superOverride)) {
        override = config.schemePunk[currentValue].superOverride;
        // eslint-disable-next-line import/no-dynamic-require
        superConfig[currentValue] = tryRequire.resolve(override) ? require(override) : superConfig[currentValue];
      }
    }
  }, this);
}
