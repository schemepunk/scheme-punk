...
const schemePunkPlugins = {
  destinationPlugins: {
    destroyDestination:
      defaultPluginRequireDirectory.destination.destroyDestination(
        superConfig.destination
      ),
    pushDestination:
      defaultPluginRequireDirectory.destination.pushDestination(
        superConfig.destination
      ),
    mergeIntoDestination:
      defaultPluginRequireDirectory.destination.mergeIntoDestination(
        superConfig.destination
      ),
    concatIntoDestination:
      defaultPluginRequireDirectory.destination.concatIntoDestination(
        superConfig.destination
      )
  },
  sourcePlugins: {
  ...
  transformPlugins: {
    appendValuesAdapter:
      defaultPluginRequireDirectory.transform.typeAdapter(
        defaultPluginRequireDirectory.transform.appendValues(
          superConfig.transform
        )
      ),
    delimitValues:
      defaultPluginRequireDirectory.transform.delimitValues(
        superConfig.transform
      ),
    fiterAttribute:
      defaultPluginRequireDirectory.transform.filterAttributes(
        superConfig.transform
      ),
    fiterAttributeMulti:
      defaultPluginRequireDirectory.transform.typeAdapter(
    ...
    objectKeysTransform:
      defaultPluginRequireDirectory.transform.objectKeysTransform(
        superConfig.transform
      ),
    prependValuesAdapter:
      defaultPluginRequireDirectory.transform.typeAdapter(
        defaultPluginRequireDirectory.transform.prependValues(
          superConfig.transform
        )
      ),
    regexWordBoundariesValues:
      defaultPluginRequireDirectory.transform.regexWordBoundariesValues(
        superConfig.transform
      ),
    tokenTemplateValues:
      defaultPluginRequireDirectory.transform.tokenTemplateValues(
        superConfig.transform
      ),
