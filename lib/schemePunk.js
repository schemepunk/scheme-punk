...
module.exports = class schemePunk {
  ...
  async enhance(schemePunkScheme) {
    ...
    try {
      ...
      const SchemeSource = await schemePunkSourceBase(this.options.source.plugin, this.molotovConfig); // eslint-disable-line max-len
      ...
      throw new SchemePunkErrors(`${SCHEME_PUNK_SOURCE_FAILURE}: ${error.message}`);
    }
    try {
      ...
      this.logger.error(error.message);
      throw new SchemePunkErrors(`${SCHEME_PUNK_TRANSFORM_FAILURE}: ${error.message}`);
    }
    ...
    try {
      ...
      this.logger.error(error.message);
      throw new SchemePunkErrors(`${SCHEME_PUNK_DESTINATION_FAILURE}: ${error.message}`);
    }
  }
